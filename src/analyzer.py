...
> - 风险分析仅关注：**指数走势、跟踪误差、市场流动性**；严禁将基金公司诉讼/高管变动/声誉纳入风险警报
> - 核心结论必须服务于「长期持有 + 逢低分批加码」策略，**严禁输出**"止损价""短线卖出""追高风险"等短线择时语言
> - 用当前价格相对近1-3年价格区间的位置（相对低点／中位／高点）判断本次是否为加码时机，而非涨跌动能或技术形态
> - `entry_price` 字段应理解为「本次分批加码的合理价位区间」，不是「买入后止损」的短线概念；无需给出止损价
> - 若近期价格并未处于相对低点，应明确给出「维持定期定额、暂不额外加码」的结论，不得为了给建议而编造买卖点
"""
        elif asset_type == 'dividend_etf':
            prompt += """
> ⚠️ **高股息 ETF 分析约束**：该标的为高股息型 ETF，核心关注配息品质与殖利率吸引力，而非价格动能。
> - 风险分析仅关注：**成分股获利品质是否支撑配息、指数流动性**；严禁将基金公司诉讼/高管变动纳入风险警报
> - 结论需说明目前价格隐含殖利率相对其历史区间是否具吸引力；若新闻中提及除息日/配息公告，需纳入判断
> - 避免使用"多头排列""突破买点"等短线技术语言，改以「目前是否为分批建仓/加码的合理价位」表述
> - 配息资讯（殖利率、除息日）若数据不足，必须明确说明「配息资讯不足，仅供参考」，严禁编造具体数字
"""
        elif asset_type == 'bond_etf':
            prompt += """
> ⚠️ **债券 ETF 分析约束**：该标的为债券型 ETF，价格主要受**利率**驱动，分析框架与一般股票/股票型ETF完全不同。
> - **严禁使用**涨跌停、量价关系、多头排列、筹码分布等股票分析语言，这些概念不适用于债券
> - 风险分析仅关注：**存续期(duration)对利率变动的敏感度、殖利率(YTM)走势、信用利差、流动性**
> - 核心结论应聚焦：目前价格/殖利率是否已反映市场对未来利率路径（如央行政策）的预期
